...
def main():
    ...
    with open(fich, 'rb') as f:
        ini = bytearray(f.read())

    codec =HuffmanCodec.from_data(ini)
    
    y=codec.encode(ini)
    with open('huffmanComprimido.hf', 'wb') as f:
        f.write(bytearray(y))
    z=codec.decode(y)
    del ini[-1]
    print(len(ini))
    print(len(bytearray(z)))
	
    with open('file1232.txt', 'wb') as f:
        f.write(bytearray(z))
    ...
    compress_ratio=(float(len(ini)) - float(len(y))) / float(len(ini))

    print("Taxa de compressao:")
    print(compress_ratio)
    print("tempo de compressao:")
    print(end-start)
