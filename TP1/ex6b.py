...
def ex6b(fichQuery,fichTarget):
    query,alfabeto,entropia=lerAudio(fichQuery)
    target,alf,entr=lerAudio(fichTarget)
    passo=int(len(query)/4)
    informacaoMutua(query,target,alfabeto,passo)

def main():
    ex6b("guitarsolo.wav","target02 - repeatNoise.wav")
